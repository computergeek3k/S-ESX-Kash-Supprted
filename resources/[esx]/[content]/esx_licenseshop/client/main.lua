...
function OpenLicenseShop()
	...
	if Config.AdvancedWeaponShop then
		if not ownedLicenses['weapon_melee'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_weapon_melee'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Melee))), value = 'buy_license_melee'})
		end

		if not ownedLicenses['weapon_handgun'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_weapon_handgun'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Handgun))), value = 'buy_license_handgun'})
		end
		...
		if not ownedLicenses['weapon_sniper'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_weapon_sniper'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Sniper))), value = 'buy_license_sniper'})
		end
	end
	...
	if Config.DMVSchool then
		if Config.SellDMV then
			if not ownedLicenses['dmv'] then
				table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_dmv'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.DriversP))), value = 'buy_license_driversp'})
			end
		end

		if not ownedLicenses['drive_truck'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_drive_truck'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Commercial))), value = 'buy_license_commercial'})
		end

		if not ownedLicenses['drive'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_drive'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Drivers))), value = 'buy_license_drivers'})
		end

		if not ownedLicenses['drive_bike'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_drive_bike'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Motorcycle))), value = 'buy_license_motorcycle'})
		end
	end

	if Config.Drugs then
		if not ownedLicenses['weed_processing'] then
			table.insert(elements, {label = ('%s - <span style="color: green;">%s</span>'):format(_U('license_weed_processing'), _U('shop_menu_item', ESX.Math.GroupDigits(Config.Prices.Weed))), value = 'buy_license_weed'})
		end
	end
